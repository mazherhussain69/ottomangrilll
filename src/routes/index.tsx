...
import { ArrowUp, ChevronDown, ChevronLeft, ChevronRight, Flame, Instagram, MapPin, Menu, Phone, ShieldCheck, Sparkles, UtensilsCrossed, X } from "lucide-react";
  ...
  return <>
    ...
    <main>
      ...
      <section id="about" className="relative overflow-hidden bg-background py-24 sm:py-32"><div className="mx-auto grid max-w-7xl gap-14 px-5 lg:grid-cols-2 lg:items-center lg:px-8">
        ...
        <div><SectionTitle eyebrow="Our story" title="A Taste of Turkish Tradition"/><p className="reveal max-w-xl text-base leading-8 text-muted-foreground">Ottoman Turkish Grill and Cuisine brings the warmth of Turkish dining to every table. Our menu celebrates halal Turkish cuisine through kebabs, doner, pide and generous grill plates prepared for sharing.</p><p className="reveal mt-4 max-w-xl text-base leading-8 text-muted-foreground">From a quick doner sandwich to an Ottoman mixed grill, every visit is an invitation to gather, share and enjoy authentic Turkish flavors.</p><div className="reveal mt-8 grid grid-cols-3 gap-2">{[[ShieldCheck,"Halal"],[UtensilsCrossed,"Turkish cuisine"],[Sparkles,"Authentic flavors"]].map(([Icon,label]) => <div key={label as string} className="border border-border bg-card px-2 py-5 text-center"><Icon className="mx-auto mb-3 text-primary" size={23}/><p className="text-[10px] font-bold uppercase tracking-[.12em]">{label as string}</p></div>)}</div></div>
      </div></section>
      ...
      <section className="pattern bg-ink py-24 text-cream sm:py-32"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle eyebrow="The Ottoman table" title="Why Choose Ottoman" light/><div className="grid gap-px bg-cream/10 sm:grid-cols-2 lg:grid-cols-4">{[[ShieldCheck,"Halal","Authentic halal dining experience."],[UtensilsCrossed,"Turkish Cuisine","Inspired by rich Turkish culinary traditions."],[Flame,"Authentic Flavors","A memorable Turkish dining experience."],[Sparkles,"Welcoming Atmosphere","A comfortable place to enjoy Turkish cuisine."]].map(([Icon,t,d],i) => <article key={t as string} className="reveal group bg-ink p-8 transition hover:bg-primary" style={{ transitionDelay:`${i*70}ms` }}><Icon size={28} className="mb-12 text-primary transition group-hover:text-primary-foreground"/><h3 className="font-display text-3xl">{t as string}</h3><p className="mt-3 text-sm leading-6 text-cream/60 group-hover:text-cream/80">{d as string}</p></article>)}</div></div></section>
    ...
    {lightbox !== null && <div role="dialog" aria-modal="true" aria-label="Food gallery" className="fixed inset-0 z-[200] grid place-items-center bg-ink/95 p-4 backdrop-blur-md"><button onClick={() => setLightbox(null)} aria-label="Close gallery" className="absolute right-5 top-5 grid h-12 w-12 place-items-center border border-cream/20 text-cream hover:bg-primary"><X/></button><button onClick={() => setLightbox((lightbox-1+gallery.length)%gallery.length)} aria-label="Previous image" className="absolute left-4 z-10 grid h-12 w-12 place-items-center bg-cream/10 text-cream hover:bg-primary"><ChevronLeft/></button><figure className="max-w-5xl animate-scale-in"><img src={gallery[lightbox].image} alt={gallery[lightbox].name} className="max-h-[78vh] w-full object-contain"/><figcaption className="mt-4 text-center font-display text-3xl text-cream">{gallery[lightbox].name} <span className="ml-3 font-sans text-base text-primary">{gallery[lightbox].price}</span></figcaption></figure><button onClick={() => setLightbox((lightbox+1)%gallery.length)} aria-label="Next image" className="absolute right-4 z-10 grid h-12 w-12 place-items-center bg-cream/10 text-cream hover:bg-primary"><ChevronRight/></button></div>}
  </>;
}